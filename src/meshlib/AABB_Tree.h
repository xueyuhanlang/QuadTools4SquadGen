...
float findNearstPoint(Vector p, Vector &nearestP,int *face_id=nullptr);
float findNearstPoint(Vector p, Vector &nearestP, int *segment_id = nullptr);
...
#define Enable_AABB_Segment_KDTree 1
    ...
    */
    ...
    namespace aabb_impl
    {
        ...
        public:
            AABB();
            AABB(const Vector *vertex_list, ptrdiff_t num);
            AABB(const std::vector<Vector> &vertices_list);
            Vector getCenter();
        ...
        protected:
            ...
            void BuildTree(const std::vector<Vector> &vertexList, const std::vector<ptrdiff_t> &face_id_list, AABBTreeInfo &treeInfo, int depth);

        public:
            Vector min, max;
            AABBNode<T> *left, *right;
            std::vector<Vector> m_pVerts; // vertices stored within this node
    ...
    public:
        ...
        void find_Nearest_Point_by_traverse(const Vector &p, Vector &nearestP, ptrdiff_t *face_ID = nullptr);
        Vector get_normal(ptrdiff_t face_ID);

    protected:
        T point_line_distance(const Vector &p, const Vector &a, const Vector &b, Vector &nearestP, T ref_dist);
        bool inside_segment(const Vector &p, const Vector &a, const Vector &b);
        void Traverse(aabb_impl::AABBNode<T> *node);
        T point_AABB_distance(const Vector &p, const aabb_impl::AABB<T> &box);
        void Traverse_Search(aabb_impl::AABBNode<T> *node, const Vector &p, Vector &globalVector, T &global_min_dist, ptrdiff_t &globalFaceID);
        T search_entire_node(aabb_impl::AABBNode<T> *node, const Vector &p, Vector &globalVector, T &global_min_dist, ptrdiff_t &globalFaceID);
        T point_tri_distance_refine(const Vector &p, const Vector &a, const Vector &b, const Vector &c, Vector &nearestP, T ref_dist);
    ...
    private:
        ...
        std::vector<Vector> vertices_list_copy;
