...
bool has_nonmanifold_issue(size_t num_vertices, const std::vector<std::vector<size_t>> &face_indices)
{
    ...
    edge_map.reserve(face_indices.size() * 2);
    for (ptrdiff_t f = 0; f < static_cast<ptrdiff_t>(face_indices.size()); ++f)
    {
        ...
        for (size_t i = 0; i < face.size(); ++i)
        {
            ...
            if (eiter == edge_map.end())
                edge_map.emplace(edge, std::make_pair(f, -1));
    ...
    for (size_t i = 0; i < vertex_ring.size(); ++i)
    {
        ...
        while (!edge_queue.empty())
        {
            ...
            edge_queue.pop();
            if (!visited_edges.emplace(edge).second)
                continue;
...
bool merge_boundary_vertices(std::vector<TinyVector<Real, 3>> &vertices, std::vector<std::vector<size_t>> &fInd, bool &nonmanifold_issue, bool fast_mode, bool skip_degenerate_faces)
{
    ...
    edge2facemap.reserve(fInd.size() * 2);
    for (size_t i = 0; i < fInd.size(); ++i)
    {
        const auto &face = fInd[i];
    ...
    const auto unique_pt_counter = MergeSamePoints<Real>(mesh_boundary_points, merge2uniqueID_map, back2overlapID_map);
    bool merged = false;
    if (unique_pt_counter >= 0 && static_cast<size_t>(unique_pt_counter) < mesh_boundary_points.size())
    {
        std::vector<TinyVector<Real, 3>> merged_vertices;
        std::vector<std::vector<size_t>> merged_faces;
        merged_vertices.reserve(vertices.size());
        merged_faces.reserve(fInd.size());
        ...
        for (ptrdiff_t i = 0; i < unique_pt_counter; ++i)
        {
        ...
        {
            vertex_id_map[mesh_boundary_vertices_ids[i]] = static_cast<size_t>(merge2uniqueID_map[i]) + num_interior_vertices;
        }
...
void manifold_submesh_extraction(std::vector<TinyVector<Real, 3>> &vertices, std::vector<std::vector<size_t>> &fInd, const bool input_is_manifold, bool fast_mode)
{
    ...
    for (size_t fi = 0; fi < fInd.size(); ++fi)
    {
        if (face_visited[fi])
            continue;
        std::unordered_set<size_t> submesh_vertice_ids, submesh_facet_ids;
        submesh_vertice_ids.reserve(fInd[fi].size());
        submesh_facet_ids.reserve(fInd.size());
        ...
        while (!face_queue.empty())
        {
            ...
            const auto &face = fInd[new_face_id];
            for (size_t j = 0; j < face.size(); ++j)
            {
                auto v0 = face[j];
                auto v1 = face[(j + 1) % face.size()];
        ...
        if (!fast_mode)
        {
            ...
            {
                std::vector<ptrdiff_t> merge2uniqueID_map, back2overlapID_map;
                const auto unique_pt_counter = MergeSamePoints<Real>(submesh_boundary_points, merge2uniqueID_map, back2overlapID_map);

                if (unique_pt_counter >= 0 && static_cast<size_t>(unique_pt_counter) < submesh_boundary_points.size())
                {
                    ...
                    for (size_t i = 0; i < merge2uniqueID_map.size(); ++i)
                    {
                        vertex_id_map[submesh_boundary_vertices_ids[i]] = static_cast<size_t>(merge2uniqueID_map[i]) + num_interior_vertices;
                    }
    ...
    if (!fast_mode && !total_submesh_faces.empty() && total_submesh_vertices.size() < vertices.size())
    {
        bool nonmanifold;
...
size_t label_connected_components(const std::vector<TinyVector<Real, 3>> &vertices, const std::vector<std::vector<size_t>> &face_indices, std::vector<size_t> &face_submesh_ids, const size_t start_id)
{
    ...
    // build connection map
    std::unordered_map<std::pair<size_t, size_t>, std::vector<size_t>> edge2facemap;
    edge2facemap.reserve(face_indices.size() * 2);
    for (size_t i = 0; i < face_indices.size(); ++i)
    {
        const auto &face = face_indices[i];
        for (size_t j = 0; j < face.size(); ++j)
        {
            auto v0 = face[j];
            auto v1 = face[(j + 1) % face.size()];
    ...
    for (size_t fi = 0; fi < face_indices.size(); ++fi)
    {
        if (face_visited[fi])
            continue;
        std::queue<size_t> face_queue;
        ...
        while (!face_queue.empty())
        {
            ...
            face_queue.pop();
            const auto &face = face_indices[new_face_id];
            for (size_t j = 0; j < face.size(); ++j)
            {
                auto v0 = face[j];
                auto v1 = face[(j + 1) % face.size()];
