namespace ig
{
	namespace kdtree_impl
	{
		...
		fun_beg:
			...
			do
			{
				while (num_list[cur_end] >= value && cur_end > cur_beg)
					cur_end--;
				if (cur_end > cur_beg)
				{
				...
				while (num_list[cur_beg] < value && cur_end > cur_beg)
					cur_beg++;
				if (cur_end > cur_beg)
				{
		...
		ptrdiff_t KD_Tree_Node<T, Dim>::findnum(const std::vector<T> &num_list, int n)
		{
			...
			std::vector<ptrdiff_t> seq(num_list.size());
			std::vector<T> num_list_copy(num_list);
#pragma omp parallel for
			for (ptrdiff_t i = 0; i < (ptrdiff_t)num_list.size(); i++)
				seq[i] = i;
		...
		int KD_Tree_Node<T, Dim>::FindBestAxis(const std::vector<Vector> &vertexList)
		{
			...
			Vector LowerCorner = vertexList[0], UpperCorner = vertexList[0];

			for (ptrdiff_t i = 1; i < (ptrdiff_t)vertexList.size(); i++)
			{
				for (int j = 0; j < Dim; j++)
				{
					LowerCorner[j] = std::min(LowerCorner[j], vertexList[i][j]);
					UpperCorner[j] = std::max(UpperCorner[j], vertexList[i][j]);
				}
			}
			Vector center = (LowerCorner + UpperCorner) / 2;
			std::vector<ptrdiff_t> left_count(Dim, 0), right_count(Dim, 0);
			for (ptrdiff_t i = 0; i < (ptrdiff_t)vertexList.size(); i++)
			{
				for (int j = 0; j < Dim; j++)
				{
					if (vertexList[i][j] < center[j])
					{
						left_count[j]++;
					}
					else
			...
			int index = 0;
			ptrdiff_t result = left_count[0] - right_count[0];
			if (result < 0)
				result = -result;
			for (int i = 1; i < Dim; i++)
			{
				ptrdiff_t value = left_count[i] - right_count[i];
				if (value < 0)
					value = -value;
				if (result > value)
				{
			...
			// 	}
			// }
			// Vector Range = UpperCorner - LowerCorner;
		...
		void KD_Tree_Node<T, Dim>::BuildTree(const std::vector<Vector> &vertexList, const std::vector<ptrdiff_t> &face_id_list, int depth)
		{
			...
			if (bMakeChildren)
			{
				...
				std::vector<T> num_list(vertexNum);
				for (int i = 0; i < vertexNum; i++)
					num_list[i] = vertexList[i][iAxis];
				...
				for (int i = 0; i < vertexNum; i++)
				{
					...
					{
						leftCount++;
						leftSide.emplace_back(vertexList[i]);
						leftSide_faceID.emplace_back(face_id_list[i]);
					}
					else
					{
						rightCount++;
						rightSide.emplace_back(vertexList[i]);
						rightSide_faceID.emplace_back(face_id_list[i]);
					}
				}
		...
		void KD_Tree_Node<T, Dim>::BuildTree(const std::vector<Vector> &vertexList, int depth)
		{
			...
			if (bMakeChildren)
			{
				...
				num_list.clear();
				for (int i = 0; i < vertexNum; i++)
					num_list.emplace_back((T)vertexList[i][iAxis]);
	...
	void KD_Tree<T, Dim>::find_Nearest_Point(const Vector &query, TinyVector<T, Dim> &nearestP, ptrdiff_t *primitive_id)
	{
		...
		search_info.cur_min_distance.emplace_back((cur_leaf_node->vertex - query).Length());
		search_info.cur_nearest_vertex.emplace_back(cur_leaf_node);
