...
{
public:
    QCDF2QuadMesh(MeshLib::Mesh3D<Real> *mesh, const std::string &featurefile, bool pattern_subdiv);
    ~QCDF2QuadMesh();
    ...
    void export_mesh(const FaceClusterType type, const std::string &filename);
...
protected:
    ...
    void quad_extraction(const FaceClusterType method);
...
private:
    ...
    void subdivide_trimesh(const int level);

    void color_pattern_subdivision();

    int cluster_via_polarization(const FaceClusterType type);
    void process_uncluster_faces(const FaceClusterType type, const int num_clusters);
    bool handle_incorrect_clusters(const FaceClusterType type, int &num_clusters);
    void process_unlabeled_faces();
    int reindex_clusters();
    Real distance_to_cluster(const ptrdiff_t fid, const ptrdiff_t cluster_centerface_id, const FaceClusterType type);
    void dcdf_build_quad_patches();
    ...
    int identify_seed_faces(const FaceClusterType type, std::vector<FaceClusterType> &seed_face_tags);
    void face_clustering_via_seeds(const FaceClusterType type, const std::vector<FaceClusterType> &seed_face_tags);
    bool identify_boundary_dcdf_vertex(const std::vector<Real> &boundary_dcdf_values, const ptrdiff_t curve_point_index);

    void merge_clusters(const FaceClusterType type);
...
protected:
    ...
    FaceClusterType input_method_type = FaceClusterType::FC_UNDEFINED;
    ...
    std::vector<TinyVector<Real, 3>> face_centroids, coffset, doffset, aoffset, boffset, v_coffset, v_doffset;
    std::vector<Real> face_areas;
    TinyVector<Real, 3> inverse_rotation[3], normalization_center;

    std::vector<int> tri_mesh_vertex_cdfdcdf_types; // 0: mixed, 1: CDF, 2: DCDF
